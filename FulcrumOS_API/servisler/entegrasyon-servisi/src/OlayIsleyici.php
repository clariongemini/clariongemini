...
class OlayIsleyici {
    ...
    public function isle($olayVerisi) {
        ...
        try {
            ...
            throw $e; // Hatanın tekrar fırlatılması, RabbitMQ'da NACK için önemli
        }
    }
    ...
    private function siparisKargolandiIsle($olay) {
        ...
        $donusturulmusVeri = [
            ...
            "urun_satirlari" => array_map(function($urun) {
                return ["sku" => $urun['sku'], "urun_adi" => $urun['urun_adi'], "adet" => $urun['adet'], "birim_fiyat" => $urun['birim_fiyat'], "kdv_orani" => $urun['kdv_orani'], "toplam_tutar" => $urun['toplam_tutar']];
            }, $olay['siparis']['urunler']),
            "toplamlar" => ["ara_toplam" => $olay['siparis']['toplamlar']['ara_toplam'], "toplam_kdv" => $olay['siparis']['toplamlar']['toplam_kdv'], "genel_toplam" => $olay['siparis']['toplamlar']['genel_toplam']]
        ];
    ...
    private function iadeOdemeBasariliIsle($olay) {
        ...
        $donusturulmusVeri = [
            ...
            "toplamlar" => ["genel_toplam" => $olay['iade']['toplam_iade_tutari']]
        ];
    ...
    private function tedarikMalKabulYapildiIsle($olay) {
        ...
        $donusturulmusVeri = [
            ...
            "islem_tarihi" => date('c'),
            "satici_bilgileri" => ["firma_unvani" => $olay['tedarik']['tedarikci_adi']],
            "musteri_bilgileri" => $saticiBilgileri,
            "urun_satirlari" => array_map(function($urun) {
                return ["sku" => $urun['sku'], "urun_adi" => $urun['urun_adi'], "adet" => $urun['gelen_adet'], "birim_fiyat" => $urun['maliyet'], "kdv_orani" => $urun['kdv_orani'], "toplam_tutar" => $urun['gelen_adet'] * $urun['maliyet']];
            }, $olay['tedarik']['gelen_urunler']),
            "toplamlar" => ["genel_toplam" => $olay['tedarik']['toplam_maliyet']]
        ];
    ...
    private function muhasebeLogKaydet($olayTipi, $referansId, $veri) {
        ...
        $stmt = $this->db->prepare("INSERT INTO muhasebe_loglari (olay_tipi, referans_id, durum, olusturulan_xml_veya_json) VALUES (?, ?, 'beklemede', ?)");
        $stmt->execute([$olayTipi, $referansId, $jsonVeri]);
    }

    private function getSaticiBilgileri() {
        // Gerçek uygulamada bu, Organizasyon-Servisi'ne internal bir API çağrısı yapar.
        return ["firma_unvani" => "FulcrumOS Satış A.Ş.", "vergi_dairesi" => "Büyük Mükellefler", "vergi_no" => "1234567890", "firma_adresi" => "İstanbul, Türkiye"];
    }
}
